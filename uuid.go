...
// NewV1, NewV3, NewV4, NewV5 and Parse() for generating versions 1, 3, 4
// and 5 UUIDs as specified in RFC 4122.
//
// Copyright (C) 2014 by Lotus <iwulove@gmail.com>
package uuid
...
const epochStart = 0x01b21dd213814000
...
// Generate UUID based on current timestamp and MAC address.
func NewV1() (u *UUID) {
	u = new(UUID)
	timeNow, seq := getTime()

	time_low := uint32(timeNow)
	time_mid := uint16(timeNow >> 32)
	time_hi := uint16(timeNow >> 48)

	binary.BigEndian.PutUint32(u[0:], time_low)
	binary.BigEndian.PutUint16(u[4:], time_mid)
	binary.BigEndian.PutUint16(u[6:], time_hi)
...
func getHardwareAddr() (hardwareAddr [6]byte) {
	inets, err := net.Interfaces()
	if err == nil {
		var set bool
		for _, inet := range inets {
			if len(inet.HardwareAddr) >= 6 {
				copy(hardwareAddr[:], inet.HardwareAddr)
				set = true
				break
			}
		}
	...
	// Initialize hardwareAddr randomly that real network interfaces absence
